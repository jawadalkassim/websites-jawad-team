...
include_once(__DIR__.'/../resources/health-insurance/form-options.php');
				...
				<div class="pt-8 sm:pt-16 pb-8">
					...
					<div class="py-8 px-0 mx-auto max-w-4xl text-center grid grid-cols-1 sm:grid-cols-2 gap-4">
						...
						<?php
							...
							foreach ($house_hold_income as $key => $value) {//$unsecured_debt_options / $unsecured_debt_amounts

								if($_SESSION['house_hold_income'] == $value['value']){
									echo '<button type="button" class="flex items-center justify-center bg-custom-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-xl md:text-2xl font-semibold border-b border-b-4 border-b-custom-dark-blue" data-target="house_hold_income" data-value="'.$value['value'].'" data-action="next-step">'.$value['text'].'</button>';
								} else {
									echo '<button type="button" class="flex items-center justify-center bg-custom-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-xl md:text-2xl font-semibold border-b border-b-4 border-b-custom-dark-blue" data-target="house_hold_income" data-value="'.$value['value'].'" data-action="next-step">'.$value['text'].'</button>';
								}
						...
						<input type="hidden" id="house_hold_income" name="house_hold_income" value="<?php if ($_SESSION['house_hold_income']) echo $_SESSION['house_hold_income']; ?>">
...
<h2 class="text-gray-700 text-3xl sm:text-4xl leading-8 font-extrabold text-center transition ease-in-out duration-300">Do You Have Any of the Following Health Conditions?</h2>
<p class="mt-2 text-gray-700 text-md sm:text-lg leading-6 font-normal text-center transition ease-in-out duration-300 hidden"></p>
<div class="flex justify-between pt-8 flex-wrap max-[350px]:justify-center">
  <ul class="list-disc ">
    <li>AIDS / HIV</li>
    <li>Asthma</li>
...
<div class="py-8 px-0 mx-auto max-w-4xl text-center grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <?php
        ...
        foreach ($health_conditions as $key => $value) {//$unsecured_debt_options / $unsecured_debt_amounts

            if($_SESSION['health_conditions'] == $value['value']){
                echo '<button type="button" class="flex items-center justify-center bg-custom-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-2xl md:text-3xl font-bold border-b border-b-4 border-b-custom-dark-blue" data-target="health_conditions" data-value="'.$value['value'].'" data-action="next-step">'.$value['text'].'</button>';
            } else {
                echo '<button type="button" class="flex items-center justify-center bg-custom-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-2xl md:text-3xl font-bold border-b border-b-4 border-b-custom-dark-blue" data-target="health_conditions" data-value="'.$value['value'].'" data-action="next-step">'.$value['text'].'</button>';
            }
    ...
    <input type="hidden" id="health_conditions" name="health_conditions" value="<?php if ($_SESSION['health_conditions']) echo $_SESSION['health_conditions']; ?>">
...
<div class="mx-auto max-w-lg font-sm pt-4 sm:pt-8">
    <div class="flex items-start w-full">
        ...
        <div class="ml-3 agent-chat">
            <div class="bg-gray-100 p-3 rounded-lg shadow-xs w-fit">
                <p class="text-sm sm:text-md text-gray-600">Different plans may offer diffrent rates based on gender.</p>
            </div>
            <p class="ml-1 mt-1 text-gray-400 text-xs sm:text-sm">Sarah, <span class="current-time"></span></p>
        </div>
    </div>
