<div class="pt-8 sm:pt-16 pb-8">
    ...
    <h2 class="text-gray-700 text-3xl sm:text-4xl leading-8 font-extrabold text-center transition ease-in-out duration-300">Which company do you currently use?</h2>
    ...
    <div class="py-8 px-0 mx-auto max-w-4xl text-center grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <?php
            ...
            foreach ($auto_insurance_companies as $key => $value) {//$unsecured_debt_options / $unsecured_debt_amounts

                if($_SESSION['currentAutoInsurance'] == $value['value']){
                    echo '<button type="button" class="flex items-center justify-center bg-custom-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-2xl md:text-3xl font-bold border-b border-b-4 border-b-custom-dark-blue" data-target="currentAutoInsurance" data-value="'.$value['value'].'" data-action="next-step">'.$value['text'].'</button>';
                } else {
                    echo '<button type="button" class="flex items-center justify-center bg-custom-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-2xl md:text-3xl font-bold border-b border-b-4 border-b-custom-dark-blue" data-target="currentAutoInsurance" data-value="'.$value['value'].'" data-action="next-step">'.$value['text'].'</button>';
                }
        ...
        <input type="hidden" id="currentAutoInsurance" name="currentAutoInsurance" value="<?php if ($_SESSION['currentAutoInsurance']) echo $_SESSION['currentAutoInsurance']; ?>">
        ...
        <div class="flex items-start w-full">
            ...
            <div class="ml-3 agent-chat">
                <div class="bg-gray-100 p-3 rounded-lg shadow-xs w-fit">
                    <p class="text-sm sm:text-md text-gray-600">Let us know who your current provider is. Or select 'Other' if you don't see them listed.</p>
                </div>
