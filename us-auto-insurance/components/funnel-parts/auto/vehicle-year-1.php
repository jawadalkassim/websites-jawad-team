...
<div class="pt-8 sm:pt-16 pb-8">
    ...
    <div class="vehicle-1-year-list py-8 px-0 mx-auto max-w-4xl text-center grid grid-cols-4 sm:grid-cols-8 md:grid-col-8 gap-4">
        ...
        <?php
            ...
            foreach($reverse_year_array as $key => $value){
                ...
                if($vehicle_year_count <= 40){
                    ...
                    if($_SESSION['vehicleYear1'] == $value){
                        echo '<button type="button" class="flex items-center justify-center bg-custom-dark-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-xl md:text-2xl font-bold border-b border-b-4 border-b-custom-dark-blue" data-target="vehicleYear1" data-value="'.$value.'" data-action="next-step">'.$value.'</button>';
                    } else {
                        echo '<button type="button" class="flex items-center justify-center bg-custom-blue text-white py-4 px-4 font-bold rounded-md w-full text-white text-xl md:text-2xl font-bold border-b border-b-4 border-b-custom-dark-blue" data-target="vehicleYear1" data-value="'.$value.'" data-action="next-step">'.$value.'</button>';
                    }
        ...
        <input type="hidden" id="vehicleYear1" name="vehicleYear1" value="<?php if ($_SESSION['vehicleYear1']) echo $_SESSION['vehicleYear1']; ?>">
