...
$(document).ready(function(){
    ...
    for (let i = 1; i < stepCount; i++) {
        $('#Step' + i + ' button[data-action="next-step"]').on('click', function () {
			...
			if(dataTarget == 'total_debt_amount'){
				$('.total-debt-slider-output').html( '$' + Intl.NumberFormat().format(dataValue) );
			}
			//
			if($(this).attr('button-type') == 'icon'){
				$(this).parent().find('button[data-action="next-step"]').removeClass('scale-100 md:shadow-blue-500/30 border-blue-400 shadow-md border-2').addClass('scale-95 md:shadow-blue-500/0 border-blue-200 shadow-sm border-1');
				$(this).removeClass('scale-95 md:shadow-blue-500/0 border-blue-200 shadow-sm border-1').addClass('scale-100 border-blue-400 shadow-md md:shadow-blue-500/30 border-2');
			}  else if($(this).attr('button-type') == 'icon-custom-color'){
				$(this).parent().find('button[data-action="next-step"]').removeClass('scale-100 shadow-blue-500/30').addClass('scale-95 shadow-blue-500/10');
				$(this).removeClass('scale-95 shadow-blue-500/10').addClass('scale-100 shadow-blue-500/30');
			} else {
	...
	$('.life_insurance_reasons input[type="checkbox"]').change(function() {
        var checkedCheckboxes = $('.life_insurance_reasons input[type="checkbox"]:checked');
	...
	$('.prescribed_medication_options input[type="checkbox"]').change(function() {
        var checkedCheckboxes = $('.prescribed_medication_options input[type="checkbox"]:checked');
		var values = checkedCheckboxes.map(function() {
            return $(this).val();
        }).get().join(',');
...
/*window.addEventListener('popstate', function(event) {
    if (event.state && event.state.page) {
        // Load the page based on the state data
...
function partialSubmit(){
	...
	$.ajax({
		url: "/resources/life-insurance/process.php?method=Partial",
		type: "post",
		data: values,
		dataType: "json",
		success: function(data) {
