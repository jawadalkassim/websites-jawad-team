...
function getFormData(formElement) {
  ...
  for (i = 0; i < formElement.elements.length; i++) {
    ...
    if (
      sub_name.match(
        /utf|pass|billing|creditcard|cardnum|^cc|ccnum|exp|seccode|securitycode|securitynum|cvc|cvv|ssn|socialsec|socsec|csc/i
      ) == null &&
      current.type.toLowerCase() != "hidden" &&
      current.type.toLowerCase() != "password"
    ) {
      if (
        current.nodeName.toLowerCase() == "input" &&
        current.type.toLowerCase() != "submit" &&
        current.type.toLowerCase() != "password" &&
        filteredMatch(current) != true
      ) {
        if (
          ...
          current.checked
            ? eval(
                "jsonObject['" + current.name + "']=\"" + current.value + '";'
              )
            : null;
        } else {
...
function filteredMatch(current) {
  ...
  if (
    ...
    if (
      ...
      current.className.match(/acskip/i) != null ||
      current.name.match(/cc/i) != null ||
      current.name.match(/credit/) != null
    ) {
      return true;
    }
  }
...
function SendData(instance, attached) {
  ...
  if (attached.indexOf("return") != -1) {
    ...
    attached =
      attached.substring(0, returnStart) +
      attached.substring(returnEnd, attached.length);
  }

  cfUniqueVisitorID = readCookie("cf_uvid", ecookie);
  instance =
    document.getElementById(instance) == null
      ? instance
...
function checkresponse(instance, attached, request, postpone) {
  ...
  setTimeout(function () {
    if (!replied) {
      request.readyState == 4 || (now == true && request.readyState > 0)
        ? answered(instance, attached, request)
        : postpone
        ? setTimeout(function () {
...
function formAttach() {
  ...
  for (counter = 0; counter < forms.length; counter++) {
    ...
    if (
      forms[counter].className != "acskip" &&
      forms[counter].action != window.location &&
      forms[counter].action.indexOf("clickfunnels.com") == -1 &&
      forms[counter].action.indexOf("/members") == -1
    ) {
      if (!forms[counter].id) {
        ...
        if (typeof forms[counter] != "undefined") {
          console.log(
            "sending data - counter: " +
              ...
              " - id: " +
              document.getElementsByTagName("form")[counter].id
          );
...
function cf_load() {
  ...
  if (
    readCookie(page_key) == "true" ||
    readCookie(fid.toString() + "_viewed_" + fspos.toString()) ==
      fvrs.toString()
  ) {
    ...
    if (typeof fid != "undefined") {
      if (
        readCookie(fid.toString() + "_viewed_" + (fspos - 1).toString()) ==
          fvrs.toString() &&
        readCookie(fid.toString() + "_viewed_1") != null &&
        readCookie(fid.toString() + "_viewed_1").toString() == fvrs.toString()
      ) {
