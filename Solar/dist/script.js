...
const utilityInp= document.getElementById("utility")
...
function validateForm() {
  ...
  for (i = 0; i < y.length; i++) {
    ...
    if (currentTab === 5) {
      if (utilityInp.value.length > 0) {
        return true;
      } else {
        errorInp(utilityInp, utilityError, "Utility provider should not be empty");
        addressInp.focus();
        valid = false;
      }
      addressInp.addEventListener("input", () => {
        if (addressInp.value.length > 0) {
          successInp(utilityInp, utilityError);
        } else {
          errorInp(utilityInp, utilityError, "Utility provider should not be empty");
        }
      });
...
$("#regForm").on("submit", function (e) {
  ...
  $.ajax({
    url: "/Solar/dist/process.php?method=Leady",
    type: "post",
    data: formData,
    dataType: "json",
